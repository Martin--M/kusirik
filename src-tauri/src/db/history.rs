...
pub fn query_history(conn: &Connection, profile_id: i64) -> Result<SearchResults> {
    ...
    let mut live_stmt = conn.prepare(
        "SELECT s.stream_id, s.name, s.stream_icon, s.epg_channel_id, s.category_id, s.tv_archive, s.tv_archive_duration, s.added, s.is_favorite
    ...
    let mut vod_stmt = conn.prepare(
        "SELECT s.stream_id, s.name, s.stream_icon, s.category_id, s.rating, s.container_extension, s.added, s.is_favorite
         FROM playback_history h
    ...
    let mut series_stmt = conn.prepare(
        "SELECT s.series_id, s.name, s.cover, s.category_id, s.rating, s.plot, s.cast_, s.director, s.genre, s.release_date, s.last_modified, s.is_favorite
         FROM playback_history h
